...
name = df['name']
data1 = pd.concat([data, df]).reset_index(drop=True)
name = data1['name']
...
print(data1['kmeans'].dtype)
data2 = data1[:data.shape[0]]
cluster = data2.groupby('kmeans')

print(list(cluster.groups.keys()))
data2.pop('kmeans')

df1 = data1.sort_values(['kmeans', 'popularity'], ascending=[True, False])
...
image1 = Image.open("images/musicback.jpg")
